...
app.get("/blogs/view/:blogID/", (req, res) => {
    const single = getBlog(req.params.blogID);
    res.render("view-blog.ejs", {single : single, blogID : req.params.blogID});
});
...
app.get("/blogs/view/edit-post/:blogID", (req, res) => {
    const single = getBlog(req.params.blogID);
    res.render("edit-post.ejs", {single : single, blogID : req.params.blogID});
});
...
app.post("/blogs/edit/:blogID", upload.single("image"), (req, res) => {
    
    const blog = blogs[req.params.blogID];

    if(req.file && req.file.filename !== ""){
        if(blog && blog.image){
            var currentImage = `public/images/blogs/${blog.image}`;
